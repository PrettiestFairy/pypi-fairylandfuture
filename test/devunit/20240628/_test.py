...
from fairylandfuture.modules.databases.mysql import MySQLDataSource, MySQLConnector
from fairylandfuture.utils.builder.expression import QueryBuilderSQL, InsertBuilderSQL
...
datasource = MySQLDataSource(host=host, port=port, user=user, password=password, database=database)
print("第一次查询".center(50, "="))
query_params = StructureSQLExecuteParams(QueryBuilderSQL(table).to_string())
print(f"SQL: {query_params.expression}, Params: {query_params.params}")
data = datasource.select(query_params)
print(f"Results: {data}")
# insert_params = StructureSQLExecuteParams(InsertBuilderSQL(table, ("user", "email")).to_string(), {"user": "李四", "email": "lisi@example.com"})
...
muilt_insert_params = StructureSQLInsertManyParams(
    InsertBuilderSQL(table, ("user", "email")).to_string(),
